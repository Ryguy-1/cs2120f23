...
notation " ⊤ " => Expr.top_exp -- Already added
notation " ⊥ " => Expr.bot_exp -- Already added
...
-- Extra credit answers here (true and false under all interpretations)
| Expr.top_exp,        _ => true
...
| mk (b : Bread) (cOs : Cheese ⊕ Spread)

-- Extra Credit Works Too :)
structure Sandwich' : Type := (b : Bread) (cOs : Cheese ⊕ Spread)
...
-- [Note From Class: true exp false exp => top exp bottom exp]
- add true_exp and false_exp as constructors in Expr              -- Ryland: Done
- note that we've already added concrete notation definitions     -- Ryland: Done
- add rules for evaluating these expressions to eval_expr         -- Ryland: Done
- add rules for these expressions to max_variable_index           -- Ryland: Done
