...
example : ¬False := (λ (a : False) => a)
example : ¬False := False.elim
example : ¬False := (λ (a : False) => False.elim a)
example : ¬False := (λ (a : False) => nomatch a)


-- a proof of not p is a function that if given a proof of p gives you a proof of false
example (P : Prop) : ¬(P ∧ ¬P) := (λ (pandnotp : (P ∧ ¬P))=> nomatch pandnotp)    -- equivalent to proving (P ∧ ¬P) → False
example (P : Prop) : ¬(P ∧ ¬P) :=
λ (pandnotp : (P ∧ ¬P)) => pandnotp.right pandnotp.left -- same as (p -> False) p [gives you false]
example (P : Prop) : ¬(P ∧ ¬P) :=
λ ⟨p, np⟩ => np p

-- these two do the same thing in different worlds!
example (P Q R : Prop) : (P → Q) → (Q → R) → (P → R) :=
fun pq qr => (λ p => qr (pq p))

example (α β γ : Type) : (α → β) → (β → γ) → (α → γ) :=
fun ab bg => fun a => bg (ab a)
...
/-!
Law of the Excluded Middle: "Every Proposition is Either True or Not True"
-/
example : X ∨ ¬X := _
