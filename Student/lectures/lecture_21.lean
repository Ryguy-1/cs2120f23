...
#reduce small 1
example: small 1 := Or.inr (Or.inl rfl) -- Ryland Added
...
example : small 1 := Or.inr (Or.inl rfl) -- Ryland Completed
...
-/
#check 1 ∈ small_set
#reduce 1 ∈ small_set -- Ryland: use this to figure out what underlying proposition is
...
example : 2 ∈ ev_set ∩ small_set := ⟨rfl, Or.inr (Or.inr (Or.inl rfl))⟩
...
example : 7 ∉ ev_set ∪ small_set := fun h => nomatch h -- Ryland: Figure this out

example : 7 ∈ ev_set := _ -- Stuck
example : 7 ∉ ev_set := fun h => nomatch h
...
#reduce Set.diff -- fun s t a => s a ∧ (a ∈ t → False)

example : 6 ∈ ev_set \ small_set := ⟨ rfl, λ h => nomatch h ⟩-- Ryland Note: "\" = minus
#reduce 6 ∈ ev_set \ small_set -- 0 = 0 ∧ (6 ∈ small_set → False)
