...
def string_set : Set String := {"hi", "hello"}
#reduce string_set
...
def other_s2_comprehsion : Set Nat := {n : Nat | small n}
/-!
...
example : small_set 1 := Or.inr (Or.inl rfl)
...
-/
example : 6 ∉ even_and_small_set := fun h => nomatch h
...
example : 7 ∉ ev_set ∪ small_set := fun h => nomatch h
...
example : 5 ∈ small_setᶜ := fun (h : 5 ∈ small_set) => nomatch h
...
example : 6 ∈ ev_set \ small_set := ⟨ rfl, λ (h : 6 ∈ small_set) => nomatch h ⟩
