...
-- Ryland: Set of infinitely many objects, each of which has finite depth
--  All its objects are finitely constructed
--  Infinite set of finitely constructed things
...
def d0 := solid
def d1 := shell d0
def d2 := shell d1 -- Ryland: also 'shell shell solid', and this trend continues
def d3 := shell d2 -- Ryland: Nested to depth 3
...
*solid*, otherwise, if *d* is a nested doll, then it's of the
form (shell d') for some doll, d', it must return d'. Then verify 
using #reduce that the function returns the correct answer when 
applied to *d3*.
-/
...
-- Ryland: Not calling bad_depth' on a proper subset of Doll
